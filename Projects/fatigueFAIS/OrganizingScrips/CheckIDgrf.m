...
function SubjectsToCheck = CheckIDgrf(SubjectFoldersInputData,sessionName)
...
for ff = 1:length(SubjectFoldersInputData)
    ...
    for k = 1:length(DynamicTrials)
        ...
        if ~exist([DirID fp trialName fp 'grf.xml'])
            continue
        end
        
        % load acquisition.xml
        XML = xml_read([DirID fp trialName fp 'grf.xml']);
