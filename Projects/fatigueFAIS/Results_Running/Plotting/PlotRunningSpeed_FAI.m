%BG - 2019
% PlotRunningSpeed_FAI
%Script to plot running speed and contact time to all participants 
%
%CALLBACK FUNCTIONS
...
for ss = 1: Nsubjects                                                       % run through all selected subject folders
    ...
    if exist('maxRunningVelocity.mat','file')==2
        ...
        RunningSpeed(ss+1,:)=num2cell(nan(1,9));
        RunningSpeed{ss+1,1}= CurrentSubject;
        continue
    end
    
    
    Ncol  = size(velocityMax,2);
    RunningSpeed(ss+1,2:Ncol+1)= num2cell(velocityMax);
    RunningSpeed{ss+1,1}= CurrentSubject;
...
RunningSpeed(1,2:Ncol+1)=Labels;                              %add labels
%% Plot
% format data to use in the callback function 'Plot_meanWindividual'
Channels = RunningSpeed(1,2:end);
ydata = cell2mat(RunningSpeed(2:end,2:end));
SubjectCodes = RunningSpeed(2:end,1);
...
ylb= ylabel({'Running speed (m/s)'}, 'Rotation',0);
...
saveas(gcf, sprintf('RunningSpeed-%s.jpeg',SubjectCodes{subjectIdx(1)}))
