...
% calculate the best iteration to minimise the difference in EMG and moment
% tracking

function gamma_opt = MinimiseEMGandMOMerr_BG (gammas,M_EMG,M_mom)
...
y = polyval(p,gammas);
plot(gammas,y,'g')
xlabel('gamma');xticklabels(xticks);yticklabels(yticks)
...
lg = legend('diff.RMSEs mom&act','fitted polynome',['opt.gamma = ' num2str(x)]);
