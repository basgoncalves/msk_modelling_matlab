...
baseDir = [pwd '\LeftStep'] ; % Base Directory to base results directory.
addpath(genpath('Utilities'))

% % % Fill Path names
INPUTS.trialname = 'walking_baseline1' ; 
INPUTS.forceFilePath = [baseDir '\grf.mot'] ;  % Full path of forces file
INPUTS.ikFilePath = [baseDir '\IK.mot'] ; % Full path of IK file
INPUTS.idFilePath = [baseDir '\Ante30_TD_FPA_ID.sto'] ; % Full path of ID file
...
INPUTS.modelName = '\Ante30_scaled_opt_N10_2times2392Fmax.osim' ; % model file name
geometryPath = 'C:\Users\willi\ucloud\Shared\CMBBE\Models' ; % full path to geometry folder for Model. If pointing to Geometry folder in OpenSim install, leave this field blank: []

% % % Set time for simulation % % %
INPUTS.startTime = 2.46 ;
INPUTS.endTime = 2.575 ;
...
INPUTS.deleteContralateralMuscles = true ; % replace muscles on contralateral leg with powerful reserve actuators (makes SO faster)
...
INPUTS.contralateralActuatorStrength = 1 ;
INPUTS.weightsToOverride = {'rect_fem'} ; % Overrides the general actuator weight for muscles or reserves.
...
INPUTS.overrideWeights = [1] ; % A column vector the same size as weights  
...
INPUTS.externalForceName = {'grf_l_3'} ; % nForces x 1 cell
INPUTS.applied_to_body = {'calcn_l'} ; 
INPUTS.force_expressed_in_body =  {'ground'} ;
INPUTS.force_identifier = {'ground_force_3_v'} ;
INPUTS.point_expressed_in_body = {'ground'} ;
INPUTS.point_identifier = {'ground_force_3_p'} ;
