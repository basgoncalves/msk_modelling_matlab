...
if exist('subjectDir','var')==0
    [subjectDir,indivData]=FindSubjectDir ();                                   % FUNCTION CALLBACK
else 
    [subjectDir,indivData]=FindSubjectDir (subjectDir);
end
...
subject = indivData(1).folder (lastDash+1:end);                            % subject name
...
PB = 6;                      % low passband frequency

%% Add anthropometric data

if exist ('AntropData.mat','file')==0                                           %find if the file AtntropData does not exists in the current folder
...
%     H = plotForce (ForceDataFiltered,fs, indivData(i).name);
%        
...
%% Convert torque to force and force to torque 
...
DataNewtons =  DataTorqueOffset ./ cell2mat (FinalData(2,:));                     % Data in Newtons = TorqueData / MomentArm
...
if isfield (TorqueDataAll,'Subjects')==0                                    % if the TorqueDataAll is empty
    ...
    TorqueDataAll.Data = zeros(1,2*length (FinalData));
...
for  i = 1:subjectRow                                                       % loop through all the Subjects
    ...
    elseif subjectRow==i                                                    % if it get to the end of the loop = current name DOES NOT exist
        ...
        TorqueDataAll.Data(subjectRow,1:end) = zeros;
...
if TorqueDataAll.Data(subjectRow,1+day)~= 0                                 % if the cell for this day (col) and subject is NOT empty (~=0)
...
for i = BegginingofLoop: 2: EndofLoop                                        %loop from
    ...
    TorqueDataAll.Data(subjectRow,finalcolumn)= DataTorqueOffset(a);               
