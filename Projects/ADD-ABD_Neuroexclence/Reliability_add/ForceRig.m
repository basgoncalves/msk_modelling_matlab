...
function TorqueDataAll = ForceRig (subjectDir)
...
save outputData indivData MaxTrials MaxTrialsLabels
...
for i = 1:length(MaxTrials)                                                 % loop through all the Max trials
    ...
    if contains (MaxTrials{1,i},'B_Eweight', 'IgnoreCase', true)  ||...
            contains (MaxTrials{1,i},'B_E_weight', 'IgnoreCase', true)      % when gets to B_Eweight
    idWeight = i;  
...
if exist ('TorqueDataAll.mat')~= 0                                          % find if TorqueDataAll.mat exists
    load TorqueDataAll
else
...
for i = BegginingofLoop: 2: EndofLoop                                        %loop from
    ...
    TorqueDataAll.Data(subjectRow,finalcolumn)= DataTorque(a);               
...
msgbox (sprintf('TorqueDataAll saved for %s',subject))
