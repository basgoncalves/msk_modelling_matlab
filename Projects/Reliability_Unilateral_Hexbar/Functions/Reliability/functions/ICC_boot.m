function [r, LB, UB, F, df1, df2, p] = ICC_boot (M1, M2, type, alpha, r0)
...
%------------------------------------------------------------------------
% UPDATE
%        03/05/2019 - ICC(A,k) according to (DOI: 10.1037/1082-989X.1.4.390)
%        01/10/2019 - Devide M in M1 and M2 to allow for formula to be used
%        with bootstraping methods

M = [M1 M2];

% if nargin < 3
%     list = {'1-1 - One-way random effects, absolute agreement, single rater/measurement',...
%         '1-k - One-way random effects, absolute agreement, multiple raters/ measurements',...
%         'C-1 - Two-way mixed effects, consistency, single rater/measurement Two-way',...
%         'C-k - Two-way mixed effects, consistency, multiple raters/measurements ICC',...
%         'A-1 - Two-way random effects, absolute agreement, single rater/measurement',...
%         'A-k - Two-way random effects, absolute agreement, multiple raters/ measurements'};
%     PromptText = sprintf ...
...
%
%     list = {'1-1','1-k','C-1',...
%         'C-k','A-1','A-k'};                       % use to match rest of
%         the fucntion
...
a = r/(n*(1-r));                                                            % UPDATED 03/05/2019
b = 1+r*(n-1)/(n*(1-r));                                                    % UPDATED 03/05/2019
v = (a*MSC + b*MSE)^2/((a*MSC)^2/(k-1) + (b*MSE)^2/((n-1)*(k-1)));

df1 = n - 1;
df2 = (n - 1) * (k - 1);                                                    % UPDATED 03/05/2019
p = 1-fcdf(F, df1, df2);
