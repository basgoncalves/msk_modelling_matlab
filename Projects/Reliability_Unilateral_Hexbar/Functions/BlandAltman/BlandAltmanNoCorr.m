...
%% Function START
function [rpc, fig, stats] = BlandAltmanNoCorr(varargin)

[fig, data, params] = ParseInputArguments(varargin{:});
[cAH, baAH,fig] = ConfigAxes(fig);
...
function [cAH, baAH,fig] = ConfigAxes(fig)
if isempty(fig)
	fig = figure;
...
params.axesLimits(3) = params.axesLimits(1);            % set the limits of the correlation graph
params.axesLimits(4) = params.axesLimits(2);
params.axesLimits(1) = min(data.maskedSet1);        
params.axesLimits(2) = max(data.maskedSet1);
