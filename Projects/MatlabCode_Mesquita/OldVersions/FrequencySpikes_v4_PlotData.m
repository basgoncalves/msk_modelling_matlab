...
for col = 1: nCol                                       % loop through all the channels
    ...
    for row = 1: length (Vector)                         % loop through all the Spikes of each channel
        ...
        if X1 ==0 || X2 == 0                             % if Spikes are different than Zero
            ...
            InstantFreq = FreqSpikes(X2,col);               % "instantaneous" frequecy
            FreqSpikesLong(X1:X2,col) = InstantFreq;
        end
    end
...
for col = 1: nCol                                       % loop through all the channels
    ...
    for row = 1: length (Vector)                         % loop through all the Spikes of each channel
        ...
        if X1 ==0 || X2 == 0                             % if Spikes are different than Zero
            ...
            InstantFreq = Frequency_Minus3(X2,col);               % "instantaneous" frequecy
...
%%  
    ...
    PolynomialData_cropped(xPol,col)= PoliFunct;
    ...
    PolynomialData_after500ms (x_500ms, col) = PoliFit_500ms;
