...
%% Start Function
function PlynomialData = FrequencySpikes_v3_PlotData
...
PolynomialData_after500ms = FreqSpikes;
...
for  col = 1: nCol                                  % loop through all the channels
    Vector = FreqSpikes (:,col);                    % plateus to use on the polynomial function
    ...
    PoliFunct = polyval(p,x);
    
    % assign each polynomial data final variable between 1.7Hz and 67Hz 
    for yourMum = 1:length(PoliFunct)
    PolynomialData_cropped (x(yourMum), col) = PoliFunct(yourMum);
    end
    % find indexes of polynomial 500ms after the beginning
    After500ms = find(x>x(1)+fs/2);
    
    x_500ms = x(After500ms);
    PoliFit_500ms =  PoliFunct(After500ms);
    
     % assign each polynomial data final variable between 1.7Hz and 67Hz
     % -AFTER 500ms of onset
    for yourDad = 1:length(PoliFit_500ms)
    PolynomialData_after500ms (x_500ms(yourDad), col) = PoliFit_500ms(yourDad);
    end
    ...
    title (sprintf('Channel %d - 1.7Hz to 64Hz after 500ms of onset', col))
    ...
    cd(ResultsFolder)
    if isempty (filename{1})~=1
    saveas(fig, sprintf('PolFit_after500ms-Channel%d.jpeg',col))
    end
    close all
end

PolynomialData_cropped(:,end+1) = ForceChannel;                                       % add the force vector at the end of PolynomialData
PolynomialData_after500ms (:,end+1) = ForceChannel; 
...
if isempty (filename{1})~=1
    ...
    cd (ResultsFolder);
    save (filename{1}, 'PolynomialData_cropped' , 'PolynomialData_after500ms', 'Frequency_Lower_17');                                              % save data with the name given
    
    %create output in a mat format
    PlynomialData = struct;
    PlynomialData.PolynomialData_cropped=PolynomialData_cropped;
    PlynomialData.PolynomialData_after500ms=PolynomialData_after500ms;
    ...
    xlswrite(filenameXls,  PolynomialData_after500ms);
