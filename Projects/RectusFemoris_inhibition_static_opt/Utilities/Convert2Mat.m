...
joints = {'hip_resultant' 'hip_x' 'hip_y' 'hip_z' 'knee_resultant' 'knee_x' 'knee_y' 'knee_z' ...
    'patellofemoral_resultant' 'patellofemoral_x' 'patellofemoral_y' 'patellofemoral_z'};
    ...
    for iAva = 1:length(AVA)
        ...
        for iPen = 1:length(penalties)
            ...
            for iFolder = 1:length(resultsDirs)
                ...
                look_for_substrings = {['hip_' l], ['knee_' l], ['ankle_' l], ['patellofemoral_' l]};
                [contactForces_data] = calc_resultant_JCF(contactForces_data,look_for_substrings,fs);


                try % in child
                    contactForces_data.(['hip_' l '_on_femur_' l '_in_femur_' l '_fx_norm']);
                    hip_field   = ['hip_' l '_on_femur_' l '_in_femur_' l];
                    knee_field  = ['walker_knee_' l '_on_tibia_' l '_in_tibia_' l ];
                catch % in parent
                    contactForces_data.(['hip_' l '_on_pelvis_in_pelvis_fx_norm']);
                    hip_field   = ['hip_' l '_on_pelvis_in_pelvis'];
                    knee_field  = ['walker_knee_' l '_on_femur_' l '_in_femur_' l];
                    patel_field = ['patellofemoral_' l '_on_femur_' l '_in_femur_' l];
                end
                
                % gather hip CF
                contactForces.(['Pen_' curr_penalty]).(joints{1})(:,end+1) = contactForces_data.(['hip_' l '_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{2})(:,end+1) = contactForces_data.([hip_field '_fx_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{3})(:,end+1) = contactForces_data.([hip_field '_fy_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{4})(:,end+1) = contactForces_data.([hip_field '_fz_norm']);
                
                % gather knee CF
                contactForces.(['Pen_' curr_penalty]).(joints{5})(:,end+1) = contactForces_data.(['knee_' l '_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{6})(:,end+1) = contactForces_data.([knee_field '_fx_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{7})(:,end+1) = contactForces_data.([knee_field '_fy_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{8})(:,end+1) = contactForces_data.([knee_field '_fz_norm']);

                % gather patelofemoral CF
                contactForces.(['Pen_' curr_penalty]).(joints{9})(:,end+1) = contactForces_data.(['patellofemoral_' l '_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{10})(:,end+1) = contactForces_data.([patel_field '_fx_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{11})(:,end+1) = contactForces_data.([patel_field '_fy_norm']);
                contactForces.(['Pen_' curr_penalty]).(joints{12})(:,end+1) = contactForces_data.([patel_field '_fz_norm']);
