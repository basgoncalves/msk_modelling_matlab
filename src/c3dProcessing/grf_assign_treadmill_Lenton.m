function [data] = grf_assign_treadmill_Lenton(data,markers,segments,FP_thresh,filt_cut)
...
for f = 1:length(data.fp_data.GRF_data)
    ...
    % define when a foot is on or off each plate - use a force threshold
    % defined aboved
    a = find(F>FP_thresh);
...
% calculate the centre of pressure as the weighted average between the COP
