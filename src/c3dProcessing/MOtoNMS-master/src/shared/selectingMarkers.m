function [markerstrc,newMarkerLabels] = selectingMarkers(MarkersList,MLabels,markers)
...
for i=1:length(MarkersList)
    ...
    if isempty(z)
        disp('Unfound marker: ')
        disp(j)
        warning(['Marker to be written in the .trc file not found in the data. Check labeled data or your selection of markers for the trc file saved in the elaboration.xml'])
    else
        if i==1
            indexMarkerLabels=[z];
        else
