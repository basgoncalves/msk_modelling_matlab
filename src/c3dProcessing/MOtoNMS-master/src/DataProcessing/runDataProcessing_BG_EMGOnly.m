...
function []=runDataProcessing_BG_EMGOnly(ElaborationFilePath,Fcut)
...
if nargin > 1
   fcut.emgbp = Fcut.EMGbp;
   fcut.emglp = Fcut.EMGlp;
end
...
if isfield(parameters,'EMGsSelected')
    ...
    %----------------------------------------------------------------------
        ...
        EMGsEnvelope=EMGFiltering(EMGsSelected,AnalogFrameRate,fcut.emgbp,fcut.emglp);
        ...
        %--------------------------------------------------------------------------
        AnalysisWindowEMG =AnalysisWindow;
        
        % add the EMG offset to the window selection of the EMG
