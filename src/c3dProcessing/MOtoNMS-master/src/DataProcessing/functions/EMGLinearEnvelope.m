function [EMGDataLinEnv] = EMGLinearEnvelope(EMGDataRaw,EMGRate,bp,lp)
%
...
BPFiltEMGPointsAll = ZeroLagButtFiltfilt((1/EMGRate), bp, 2, 'bp', EMGDataRawZeroMean);
RectBPFiltEMGPointsAll = abs(BPFiltEMGPointsAll);
LinEnvEMGAll = ZeroLagButtFiltfilt((1/EMGRate), lp, 2, 'lp', RectBPFiltEMGPointsAll);
...
function fdata = ZeroLagButtFiltfilt(dt, fcut, order, type, data,idx,WarningOn)
...
for i = 1:nCols
    ...
    if nargin>5 && ~isempty(idx)    %if we are filtering markers
