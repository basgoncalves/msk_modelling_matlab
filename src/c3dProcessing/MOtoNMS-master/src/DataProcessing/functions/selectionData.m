...
for k=1:length(filtData)
    ...
    %(that is the lowest)
    DataAnalysisWindow{k}.startFrame=round(((AnalysisWindow{k}.startFrame)/AnalysisWindow{k}.rate-offset)*Rate)-Rate/AnalysisWindow{k}.rate+1;
    %conversion of endFrame into Rate
    DataAnalysisWindow{k}.endFrame=round(((AnalysisWindow{k}.endFrame)/AnalysisWindow{k}.rate-offset)*Rate)-Rate/AnalysisWindow{k}.rate+1;
