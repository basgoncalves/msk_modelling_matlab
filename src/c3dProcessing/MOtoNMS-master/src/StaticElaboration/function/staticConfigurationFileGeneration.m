...
% Made this manual at 8Hz - can uncomment above code to manually select.
fcut = 8;
...
for j=1:size(dirList,1)                         %dirList(1,:) always = '.' 
                                                %dirList(2,:) always = '..'
    jointList{j}=regexprep(dirList(j).name, ' ' , '');
end
...
% Manually defined joint centres. If you want to select from full list then
% comment next line and uncomment the listdlg above
jointCenters = {'Hip', 'Knee', 'Ankle'};
