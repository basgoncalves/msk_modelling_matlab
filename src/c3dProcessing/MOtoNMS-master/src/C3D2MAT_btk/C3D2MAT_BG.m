...
%
% USAGE: Input files folder path MUST included a folde  r named 'InputData'
...
%% SubjectFolder = directory of the subject / folder containing the 'InputData' folder

function []=C3D2MAT_BG(SubjectFolder,TrialList)


if isempty(TrialList)
    disp('Trial list is empty')
    TrialList =  cellstr(ls(SubjectFolder));
    TrialList(1:2) =  [];
end
...
if nargin ==0
    SubjectFolder = uigetdir('Select folder with .c3d files');
end

cd(SubjectFolder)
pathName = SubjectFolder;
...
%     disp(trialsName{k})
    ...
    if ~contains(upper(trialsName{k}),'STATIC')
