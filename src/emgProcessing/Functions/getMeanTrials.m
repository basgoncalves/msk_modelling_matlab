...
%   'HE'0.1 0.9  1  0.5
%% START FUNCTION
function [MeanTrials,SDTrials] = getMeanTrials (GroupData,labels)
...
Groups(end+1) = length(labels)+1;                             % last group of trials 
...
for ii = 2:length(Groups)
    ...
    MeanData = mean(data,1);                                           % max for each column. For rows -> max(data,[],2)
    SDData = std(data,[],1);                                           % max for each column. For rows -> max(data,[],2)

    
    MeanTrials{ii,1} = TrialName;
    MeanTrials(ii,2:Ncol+1)= num2cell(MeanData);
    
    SDTrials{ii,1} = TrialName;
    SDTrials(ii,2:Ncol+1)= num2cell(SDData);
