function emgProcessingMaxMultiple(isNotch, sessionData,  maxc3dFileNames, motoDir)
...
% Initialise matrix to store all max data. Rows correspond to separate
% trials and columns to separate EMG channels
emgMaxAll = zeros(length(maxc3dFileNames), length(emgLabelsInMatFile));

for files = 1:length(maxc3dFileNames)
	
	maxData = [sessionData, filesep, maxc3dFileNames{files}]; % Max trial dir
	...
	for l = 1:size(emgLabelsInMatFile,2)
		...
		% Add max value for condition into the first row
		emgMaxAll(files, l) = emgMax(1,l);
	end
end
...
printDir = [sessionData, filesep, 'emgMax'];
if ~isdir(printDir)
	mkdir(printDir)
end
