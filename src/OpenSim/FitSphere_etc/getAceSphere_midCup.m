...
function [center_ace, radius_ace, midcup, psup_max, center_circ] = getAceSphere_midCup(Scale_output)

%%
path = fileparts (matlab.desktop.editor.getActiveFilename); % path of the current script
cd(path)
if nargin <1
    ScaleXML = xml_read('Scale_output.xml');
    disp('###########################') 
    disp('# generating example file #') 
    disp('###########################') 
else
    ScaleXML = xml_read(Scale_output);
end
fid = fopen('acetabulum.asc', 'rt');
C = textscan(fid, '%f%f%f', 'Delimiter',' ', 'HeaderLines', 1);
fclose(fid);
scaleF = mean(ScaleXML.ScaleSet.objects.Scale(2).scales); % get the scaling factors for the pelvis
...
intersect2 = v3*double(sol(2)) + center_ace;   % vector between centre 
...
plot3(midcup(1),midcup(2),midcup(3),'.r','MarkerSize',20)
[X,Y,Z] = sphere;
X2= (X)*rad_circ+center_circ(1);
Y2= (Y)*rad_circ+center_circ(2);
Z2= (Z)*rad_circ+center_circ(3);
s=surf(X2,Y2,Z2);
plot3(center_circ(1),center_circ(2),center_circ(3),'.r','MarkerSize',20)

s.FaceColor=[0.4 0 0];
s.FaceAlpha =0.3;
figure; hold on; 
ML_axis = [center_circ;midcup];
plot3(ML_axis(:,1),ML_axis(:,2),ML_axis(:,3),'r','LineWidth',4)
V_axis = [center_circ;psup_max];
plot3(V_axis(:,1),V_axis(:,2),V_axis(:,3),'b','LineWidth',4)
AP_axis = [center_circ; pant];
plot3(AP_axis(:,1),AP_axis(:,2),AP_axis(:,3),'g','LineWidth',4)


ML_axis_rxyz = ML_axis*Rx*Ry*Rz;
plot3(ML_axis_rxyz(:,1),ML_axis_rxyz(:,2),ML_axis_rxyz(:,3),':r','LineWidth',4)
V_axis_rxyz = V_axis*Rx*Ry*Rz;
plot3(V_axis(:,1),V_axis(:,2),V_axis(:,3),':b','LineWidth',4)
AP_axis_rxyz = AP_axis*Rx*Ry*Rz;
plot3(AP_axis(:,1),AP_axis(:,2),AP_axis(:,3),':g','LineWidth',4)

axis equal

figure; title('AP vector')
quiver(ap(1,2),ap(2,2),ap(1,3),ap(2,3))
angles = 90;
x = cos(angles(i))*rad_circ;
y = sin(angles(i))*rad_circ;
pcirc(i,:) = center_circ + [x,y,0]*Rx*Ry*Rz;
...
ML_axis_rxyz = v1*Rx*Ry*Rz;
quiver3(0,0,0,ML_axis_rxyz(1),ML_axis_rxyz(2),ML_axis_rxyz(3))
axis equal
