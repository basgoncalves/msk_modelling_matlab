          ...
          for j = 1:length(on_i)
               ...
               if i == 1 && j == 1
                    if filter_freq > 0 % filter the data if a filter frequency is defined (defaults at low pass 25Hz)
                         data.GRF.FP(i).(assign_bodies{i}).F(a,:) = matfiltfilt(dt,filter_freq,2,data.fp_data.GRF_data(i).F(a,:));
                         data.GRF.FP(i).(assign_bodies{i}).M(a,:) = matfiltfilt(dt,filter_freq,2,data.fp_data.GRF_data(i).M(a,:));
                         data.GRF.FP(i).(assign_bodies{i}).P(a,:) = matfiltfilt(dt,filter_freq,2,data.fp_data.GRF_data(i).P(a,:));
               ...
               elseif i == 1 && j == 2
                    if filter_freq > 0 % filter the data if a filter frequency is defined (defaults at low pass 25Hz)
                         data.GRF.FP(i).(assign_bodies{j}).F(a,:) = matfiltfilt(dt,filter_freq,2,data.fp_data.GRF_data(i).F(a,:));
                         data.GRF.FP(i).(assign_bodies{j}).M(a,:) = matfiltfilt(dt,filter_freq,2,data.fp_data.GRF_data(i).M(a,:));
                         data.GRF.FP(i).(assign_bodies{j}).P(a,:) = matfiltfilt(dt,filter_freq,2,data.fp_data.GRF_data(i).P(a,:));
                    else % otherwise just assign the raw data
