function [Q,IQR,outliersQ1, outliersQ3] =  quartile(x)
% define data set
...
% determine extreme Q1 outliers (e.g., x < Q1 - 1.5*IQR)
iy = find(y<Q(2)-1.5*IQR);
if length(iy)>0,
    outliersQ1 = y(iy);
else
    outliersQ1 = [];
...
% determine extreme Q3 outliers (e.g., x > Q1 + 1.5*IQR)
iy = find(y>Q(2)+1.5*IQR);
if length(iy)>0,
    outliersQ3 = y(iy);
else
