...
%% LoadResults_BG
function [OrderedResults,OrderedLabels] = bops_load_results (DataDir,TimeWindow,FieldsOfInterest,MatchWholeWord,Normalise,ConvertToStruct)
...
if nargin<3 || isempty(FieldsOfInterest);  FieldsOfInterest={};end
if nargin<4 || isempty(MatchWholeWord); MatchWholeWord = 1;end                          % 1 for "yes" (default) or other for "no";
if nargin<5 || isempty(Normalise); Normalise=1;end
if nargin<6 || isempty(ConvertToStruct); ConvertToStruct=0;end
