...
% if file is empty or is not struct print empty outputs
...
% if time window is empty get data from beginning till the end
...
% deafult inputs 1 for "yes" (default) or other for "no";
if nargin<3; FieldsOfInterest={};end
if nargin<4; MatchWholeWord = 1;end                                                     
if nargin<5; Normalise=1;end
if nargin<6; ConvertToStruct=0;end

%LoadData
[results,Labels]=findData(Data.data(t(1):t(2),:),Data.colheaders,FieldsOfInterest,MatchWholeWord);      

% time normalise if required
if Normalise==1
    fs =1/(Data.data(2,1)-Data.data(1,1));  
    results = TimeNorm(results,fs);
end                             
