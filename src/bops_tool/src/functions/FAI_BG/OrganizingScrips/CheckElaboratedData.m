...
function SubjectsToCheck = CheckElaboratedData(SubjectFoldersInputData,sessionName)
...
for ff = 1:length(SubjectFoldersInputData)
    ...
    XML = xml_read([DirElaborated fp 'dynamicElaborations' fp 'elaboration.xml']);
    for k = 1:length(DynamicTrials)
        ...
        idx_type = find(contains({XML.Trials.Trial.Type},trialName(1:end-1)));
