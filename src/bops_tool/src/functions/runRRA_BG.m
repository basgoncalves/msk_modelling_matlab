function outputFile = runRRA_BG(osimModel, coordinates_file, GRFmot_file, external_loads_file, results_directory, XMLTemplate, setupFiles, varargin)
...
if exist([DirIK fp trialName fp 'GaitCycle.mat'])
    [FC, TO, GaitCycle] = GCOS(DirIK,trialName);
    fs = data.marker_data.Info.frequency;
    final_time = GaitCycle.TO_time(2);
    initial_time = GaitCycle.TO_time(1);
...
disp('')
disp('')
disp('RRA window and mass adjustments from TO to TO')
