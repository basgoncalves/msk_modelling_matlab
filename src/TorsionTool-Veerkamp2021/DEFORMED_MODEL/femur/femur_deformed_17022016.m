...
% for i = 1:length(femurNUM)
%     femurSTR = num2str(femurNUM(i,1:3));
%     femurAll{i,1} = femurSTR;
% end

% for i = 1:size(femurAll,1)
%     femurtogether = sprintf('\t\t%s\n', femurtogether, femurAll{i,1});
% end

femurtogether =[];
femurtogether=sprintf('\t\t\t%+8.6f %+8.6f %+8.6f\n',femurNUM);
nytt = strrep(femurtogether,'+',' ')
%for i = 1:size(femurAll,1)
%    femurtogether = [femurtogether,sprintf('\t\t\t%4.2f %4.2f %4.2f\n',femurNUM(i,1:3));
%end
