...
for i = 1:size(muscles,2)
    ...
    if AttachmentSize == 1
        ...
        for ii = 1:size(MuscleAttachments1_femur,2)
            CompareStrings1_femur = strcmp(femurMA, regexprep(MuscleAttachments1_femur{1,ii}.socket_parent_frame.Text,'/bodyset/',''));
    ...
    elseif AttachmentSize ==2
        ...
        if size(MuscleAttachments2_femur) == 1;
            CompareStrings2_femur = strcmp(femurMA, regexprep(MuscleAttachments2_femur.socket_parent_frame.Text,'/bodyset/',''));
    ...
    elseif AttachmentSize == 3
        ...
        if size(MuscleAttachments1_femur) == 1;
            CompareStrings1_femur = strcmp(femurMA, regexprep(MuscleAttachments1_femur.socket_parent_frame.Text,'/bodyset/',''));
            if CompareStrings1_femur == 1;
        ...
        % The muscle that have more than one conditional path point
            ...
            for ii = 1:size(MuscleAttachments2_femur,2)
                CompareStrings2_femur = strcmp(femurMA, regexprep(MuscleAttachments2_femur{1,ii}.socket_parent_frame.Text,'/bodyset/',''));
                if CompareStrings2_femur == 1;
                    femurMuscle = [femurMuscle; str2num(MuscleAttachments2_femur{1,ii}.location.Text)];
                    femurNR = [femurNR; i];
                    place1 = sprintf('ConditionalPathPoint{1,%d}',ii);
        ...
        if size(MuscleAttachments3_femur) == 1
            CompareStrings3_femur = strcmp(femurMA, regexprep(MuscleAttachments3_femur.socket_parent_frame.Text,'/bodyset/',''));
        ...
        % The muscles with more than one moving path point 
            ...
            for ii = 1:size(MuscleAttachments3_femur,2)
                CompareStrings3_femur = strcmp(femurMA, regexprep(MuscleAttachments3_femur{1,ii}.socket_parent_frame.Text,'/bodyset/',''));
                if CompareStrings3_femur == 1;
