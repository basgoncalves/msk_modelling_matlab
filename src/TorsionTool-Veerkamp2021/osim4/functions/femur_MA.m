...
for i = 1:size(muscles,2)
    ...
    if size(MuscleAttachments1) == 1
        body = strtrim(regexprep(MuscleAttachments1.socket_parent_frame.Text,'/bodyset/',''));
        ...
        for ii = 1:size(MuscleAttachments1,2)
            body = strtrim(regexprep(MuscleAttachments1{1,ii}.socket_parent_frame.Text,'/bodyset/',''));   
