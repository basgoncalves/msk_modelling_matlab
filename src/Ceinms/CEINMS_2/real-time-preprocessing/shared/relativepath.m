...
% Make sure strings end by a fp character:
if  length(act_path) == 0   |   ~isequal(act_path(end),fp)
   act_path = [act_path fp];
end
if  length(tgt_path) == 0   |   ~isequal(tgt_path(end),fp)
   tgt_path = [tgt_path fp];
end
...
for  i = 1 : length(tgt_path_cell)
   rel_path = [rel_path tgt_path_cell{i} fp];
end
...
% delete the last fp (BG)
if  contains(rel_path(end),fp)
   rel_path(end) =[];
end
...
fp = getFp();
path_str = [fp path_str fp];
path_cell = {};

sep_pos = findstr( path_str, fp );
