...
% Todo:
%       -
function writeExecutionCFGxml_TG(nmsModel, cfgExeDir, side, fileOut, pref)
%========default preferences==================================
...
s = lower(side{1});
prefDef.synthMTUs = ['iliacus_' s ' psoas_' s]; %muscles being filled with static opt
prefDef.adjustMTUs = ['recfem_' s ' vas_med_' s ' vas_lat_' s ' vas_int_' s ' bifemsh_' s ...
    ' glut_min1_' s ' glut_min2_' s ' glut_min3_' s ' glut_med1_' s ' glut_med2_' s ' glut_med3_' s ...
    ' tfl_' s ' add_brev_' s ' add_long_' s ' add_mag1_' s ' add_mag2_' s ' add_mag3_' s ' grac_' s ...
    ' sar_' s ' semiten_' s ' semimem_' s ' bifemlh_' s ' glut_max1_' s ' glut_max2_' s ' glut_max3_' s ...
    ' lat_gas_' s ' med_gas_' s ' soleus_' s ' per_long_' s ' tib_ant_' s ' per_tert_' s ' per_brev_' s]; % muscles being adjusted, leave blank if wanting to use emg inputs only
...
prefDef.gamma = 50; % gamma * EMGtracking error (sum of absolute differences between adjusted and experimental EMG-excitations)
%some might not be used as not using simulated annealing for execution in hybrid mode
prefDef.dofSet = [6 3 1 1]; % degrees of freedom for all the joints
prefDef.noEpsilon = 4;
prefDef.rt = 0.3;
prefDef.T = 20;
prefDef.NS = 15;
