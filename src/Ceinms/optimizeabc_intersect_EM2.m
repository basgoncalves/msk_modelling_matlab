function [ RMSE_sum , gamma_opt] = optimizeabc_intersect_EM2(Subj_Code, dirFolders, trial_list, MuscleNamesOS, DeepHipMuscles)
...
for w = 1:length(gammas)
    dirAssistedCfg = [dirFolders.dirExecution '\AssistedCfg-',num2str(DeepHipMuscles),'.xml'];
    ...
    for iTrial = 1:length(trial_list) % 
        ...
        dirExecutionOutputOpt = [dirExecutionOutput, 'optimization\gamma' num2str(gammas(w)),'\'];
        dirExecutionOutputOptRel = relativepath(dirExecutionOutputOpt,dirFolders.dirExecution);
%         load setup file and write new one with right output directory
        ...
        if k ==0  % files don't exist yet.            
            ...
            logFileOut = [dirExecutionOutputOpt,'\',trialname,'-gamma',num2str(gammas(w)),'.log'];
